...
public class RoomFloorSelection extends javax.swing.JFrame {

    public RoomFloorSelection() {
        initComponents();

        String query = "SELECT * FROM `room` INNER JOIN `floor` ON `floor`.`id`=`room`.`floor_id` INNER JOIN `table_room_status` ON `table_room_status`.`id`=`room`.`table_room_status_id` WHERE `floor`='First' ORDER BY `room`.`id` ASC";
        loadFloorItems(query, jPanel7);

        query = "SELECT * FROM `room` INNER JOIN `floor` ON `floor`.`id`=`room`.`floor_id` INNER JOIN `table_room_status` ON `table_room_status`.`id`=`room`.`table_room_status_id` WHERE `floor`='Second' ORDER BY `room`.`id` ASC";
        loadFloorItems(query, jPanel9);

        query = "SELECT * FROM `room` INNER JOIN `floor` ON `floor`.`id`=`room`.`floor_id` INNER JOIN `table_room_status` ON `table_room_status`.`id`=`room`.`table_room_status_id` WHERE `floor`='Third' ORDER BY `room`.`id` ASC";
    ...
    private void loadFloorItems(String query, JPanel jPanel) {
        try {
            ...
            while (tableData.next()) {
                JButton jButton = new JButton(tableData.getString("room_name"));
                ...
                roomDetails.put(tableData.getString("floor.floor"), tableData.getString("floor.id"));
                roomDetails.put(tableData.getString("room.room_name"), tableData.getString("room.id"));
                roomDetails.put("floorName", tableData.getString("floor.floor"));
                roomDetails.put("roomName", tableData.getString("room.room_name"));

                jButton.addActionListener(e -> {
                    RoomInvoice roomInvoice = new RoomInvoice(roomDetails);
                    roomInvoice.setVisible(true);
                    this.dispose();
                });
...
//            e.printStackTrace();
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new RoomFloorSelection().setVisible(true);
            }
        });
    }
