...
public class RoomInvoice extends javax.swing.JFrame {

    private static HashMap<String, String> roomMap;

    public RoomInvoice(HashMap<String, String> roomDetails) {
        initComponents();
        roomMap = roomDetails;
        loadPreviousItem();
        setDefaultComponent();
    ...
    private int checkReservation() {
        ...
        String query = "SELECT * FROM `room` INNER JOIN `table_room_status` ON `table_room_status`.`id`=`room`.`table_room_status_id` INNER JOIN `floor` ON `floor`.`id`=`room`.`floor_id` WHERE room.id='" + roomMap.get(roomMap.get("roomName")) + "' AND `floor`.`floor`='" + roomMap.get("floorName") + "' AND `table_room_status`.`status`='Pending'";
        try {
            ResultSet tableData = SQLConnector.search(query);
            if (tableData.next()) {
                query = "SELECT * FROM `room_reservation` INNER JOIN `reservation_status` ON `reservation_status`.`id`=`room_reservation`.`reservation_status_id` WHERE `reservation_status`.`status`='Pending' AND room_id='" + tableData.getString("room.id") + "'";
                ResultSet reservationTable = SQLConnector.search(query);
                if (reservationTable.next()) {
                    returnValue = reservationTable.getInt("room_reservation.id");
                } else {
                    returnValue = 0;
                }
            } else {
...
//            e.printStackTrace();
    ...
    private void loadPreviousItem() {
        ...
        if (reservationID != 0) {
            String query = "SELECT * FROM `room_reservation_item` INNER JOIN `product` ON `product`.`id`=`room_reservation_item`.`product_id` WHERE `room_reservation_item`.`room_reservation_id`='" + reservationID + "'";
            try {
...
//                e.printStackTrace();
    ...
    public void setDefaultComponent() {
        ...
        jButton7.setText(roomMap.get("floorName") + " Floor");
        jButton26.setText(roomMap.get("roomName"));
    ...
    private void jButton24ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton24ActionPerformed
        int confirmStatus = JOptionPane.showConfirmDialog(this, "Are you sure want to clear this room", "Warning", JOptionPane.YES_NO_OPTION);
        if (confirmStatus == 0) {
            String query = "DELETE FROM room_reservation_item WHERE room_reservation_id='" + reservationID + "'";
            try {
                SQLConnector.iud(query);
                query = "DELETE FROM room_reservation WHERE id='" + reservationID + "'";
                SQLConnector.iud(query);
            } catch (Exception e) {
//                e.printStackTrace();
                getLogger.logger().warning(e.toString());
    ...
    private void clearRoomStatus() {
        String query = "UPDATE `room_reservation` SET `reservation_status_id`=(SELECT `id` FROM `reservation_status` WHERE `status`='Closed' LIMIT 1) WHERE room_id='" + roomMap.get(roomMap.get("roomName")) + "' AND id='" + reservationID + "'";
        try {
            SQLConnector.iud(query);
            query = "UPDATE `room` SET `table_room_status_id`=(SELECT `id` FROM `table_room_status` WHERE `status`='Available') WHERE id='" + roomMap.get(roomMap.get("roomName")) + "'";
            SQLConnector.iud(query);
        } catch (Exception e) {
//            e.printStackTrace();
            getLogger.logger().warning(e.toString());
    ...
    private void jButton25ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton25ActionPerformed
        if (jTextField7.getText().equals("0")) {
            ...
            jPanel3.removeAll();
            jPanel3.add(new InvoicePaymentPanel(jTextField7.getText(), takeAwayVector, jTable1, this.jPanel3, jTextField2, jTextField3, jTextField5, jTextField4, jTextField7, "Room", roomMap.get(roomMap.get("roomName")), String.valueOf(reservationID), null), BorderLayout.CENTER);
            SwingUtilities.updateComponentTreeUI(jPanel3);
        }
...
//                Insert to previous reservationID
                String query = "INSERT INTO room_reservation_item (`qty`,`product_id`,`room_reservation_id`,`reservation_item_status_id`) "
...
//                Insert to new reservationID
                String query = "UPDATE `room` SET table_room_status_id=(SELECT id FROM table_room_status WHERE status='Pending') WHERE id='" + roomMap.get(roomMap.get("roomName")) + "'";
                try {
                    SQLConnector.iud(query);
                    String in_time = (new SimpleDateFormat("yyyy/MM/dd HH:mm:ss")).format(new Date());
                    query = "INSERT INTO room_reservation (room_id,in_time,reservation_status_id) "
                            + "VALUES ('" + roomMap.get(roomMap.get("roomName")) + "','" + in_time + "',(SELECT `id` FROM `reservation_status` WHERE `status`='Pending' LIMIT 1))";
                    ...
                    query = "INSERT INTO room_reservation_item (`qty`,`product_id`,`room_reservation_id`,`reservation_item_status_id`) "
    ...
    private void jTable1KeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTable1KeyReleased
        if (jTable1.getRowCount() != 0) {
            if (evt.getKeyCode() == 127) {
                ...
                String query = "DELETE FROM room_reservation_item WHERE product_id='" + productID + "' AND `room_reservation_id`='" + reservationID + "'";
                try {
                    SQLConnector.iud(query);
                } catch (Exception e) {
//                    e.printStackTrace();
    ...
    private void viewRoomSelection() {
